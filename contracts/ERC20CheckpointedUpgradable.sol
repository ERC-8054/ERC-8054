...
/**
 * @dev Upgradeable version of ERC20Checkpointed. Mirrors the non-upgradeable implementation
 * but follows OpenZeppelin upgradeable patterns.
 */
abstract contract ERC20CheckpointedUpgradable is Initializable, ContextUpgradeable, IERC20Checkpointed, IERC20Errors {
    using Checkpoints for Checkpoints.Trace208;

    /// @custom:storage-location erc7201:erc8054.storage.ERC20Checkpointed
    struct ERC20CheckpointedStorage {
        mapping(address account => Checkpoints.Trace208) _balances;
        mapping(address account => mapping(address spender => uint256)) _allowances;
        uint48 _checkpointNonce;
        Checkpoints.Trace208 _totalSupply;
        string _name;
        string _symbol;
    }

    // keccak256(abi.encode(uint256(keccak256("erc8054.storage.ERC20Checkpointed")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant ERC20CheckpointedStorageLocation =
        0x8e25f1147e50bfc8ab3ea126c894945af91200eb7bdf8762317c5ad80d8dc000;

    function _getERC20Storage() private pure returns (ERC20CheckpointedStorage storage $) {
        assembly {
            $.slot := ERC20CheckpointedStorageLocation
        }
    }
